...
const banner = '\n' +
  ...
  '  development workflows for Claude Code, OpenCode, Kimi CLI, Kilo, Codex, Copilot, Antigravity, Cursor, Grok Build, Windsurf, Augment, Trae, Qwen Code, Hermes Agent, Cline, CodeBuddy, ZCode and pi.\n';
...
function install(isGlobal, runtime = DEFAULT_RUNTIME, options = {}) {
  ...
  //
  // grok: first-class via capabilities/grok (artifactLayout converters
  // convertClaudeCommandToGrokSkill / convertClaudeAgentToGrokAgent). After
  // #2875 the fork-only `_DESCRIPTOR_AGENTS_RUNTIMES` exclusion is gone —
  // grok is a skills runtime with a non-empty layout, so agents stay on the
  // descriptor path and are not overwritten RAW.
