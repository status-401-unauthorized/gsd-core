...
const RETIRED_SUBPATHS = ['skills', 'agents'] as const;

/** Only GSD-owned top-level entries are retirement candidates. */
const GSD_PREFIX = 'gsd-';

const FILE_REASON =
  'Antigravity scans ~/.gemini/config/{skills,agents} for global discovery and does not scan the ' +
...
const FILE_OWNERSHIP_EVIDENCE =
  'pre-#3738 antigravity installs record skills/gsd-*/** and agents/gsd-*.md keys in ' +
...
const DIR_REASON =
  'the retired container directory is removed only once every GSD-owned entry inside it is gone; a ' +
...
const DIR_OWNERSHIP_EVIDENCE =
  'skills/ and agents/ at the antigravity configHome root are GSD-installed container directories ' +
 ...
 */
function walkTree(root: string, relDir: string, baseResolved: string, files: string[], dirs: string[]): void {
...
const migration: InstallerMigration = {
  ...
  description:
    'Remove manifest-managed skills/gsd-*/ and agents/gsd-*.md under the Antigravity configHome ' +
  ...
  plan: (ctx: MigrationPlanContext): MigrationAction[] => {
    ...
    for (const subpath of RETIRED_SUBPATHS) {
      const surfaceRoot = path.join(ctx.configDir, subpath);
      let rootLstat: fs.Stats;
      try {
        rootLstat = fs.lstatSync(surfaceRoot);
      } catch {
      ...
      if (rootLstat.isSymbolicLink() || !rootLstat.isDirectory()) continue;

      const baseResolved = path.resolve(ctx.configDir);
      const files: string[] = [];
      const dirs: string[] = [];
      try {
        const entries = fs.readdirSync(surfaceRoot, { withFileTypes: true });
        for (const entry of entries) {
          if (entry.isSymbolicLink() || !entry.name.startsWith(GSD_PREFIX)) continue;
