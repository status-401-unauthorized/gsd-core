...
**本节中的每个字段都是单值的，本节会被覆盖而不是追加。** 第二个 `Phase:` 行并不是第二个位置，也不是历史记录——它是格式错误的输入。读取器并不按文档中的出现顺序来解析重复项，而是按**形式**来解析：判定顺序与各形式在 `## Current Position` **本节内**的位置无关，依次是本节内任意位置的粗体 `**Phase:** value`，其次是位于行首的普通文本 `Phase: value`，最后是表格行 `| Phase | value |`——而且只有在**胜出的形式内部**，才由**第一个**出现的值取胜。本节之外的粗体或普通文本行——例如更早的 `## Archive` 条目，或 YAML frontmatter 中的粗体行——绝不会被读取；生产实现总是先将范围限定到 `## Current Position` 本节（#2956），然后才应用形式优先级。这带来两个实际后果：以更高优先级形式写入的重复项，即使在本节中出现得**更晚**也会获胜，因此为"强调"而追加的粗体行会静默覆盖在它之前的普通文本行，而不是被忽略；此外，缩进的 `Phase:` 行对普通文本形式（以行首为锚点）是不可见的，会被跳过并落到下一个匹配的形式上。"更高形式获胜"这条规则还带来两个需要留意的边界情况：如果粗体行的值只是尾随空白，则会解析为空字符串，而不会回退到下方有效的普通文本行；如果表格行的标签单元格本身是粗体写法（`| **Phase:** | value |`），会被粗体模式优先捕获，返回包含竖线在内的原始单元格文本。写入本节时应替换它，而不是新增一行。

进度*历史*不属于这里。它会累积到下方的 [`### Performance Metrics`](#性能指标) 中，那才是设计用于增长的部分。
